// +-------------------------------------------------------------------------
//
//   TypeScript Commodore 64 模拟器 - PAL 整机帧调度器
//
//   文件:       PalFrameScheduler.ts
//
//   日期:       2026年08月10日
//   作者:       OpenAI Codex
// --------------------------------------------------------------------------

import { C64Machine, type C64ClockedPeripheral } from './C64Machine';
...
export class PalFrameScheduler {
  ...
  runFrame(onRasterLine?: RasterLineCallback, checkBreakpoints = false): void {
    ...
    try {
      while (!frameCompleted) this.executeInstruction(checkBreakpoints);
    } finally {
      stopObserving();
    }
  }
